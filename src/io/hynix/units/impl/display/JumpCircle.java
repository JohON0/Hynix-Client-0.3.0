...
public class JumpCircle extends Unit {
    ...
    private ResourceLocation jumpTexture() {
        return new ResourceLocation(staticLoc + "glow" + ".png");
    }

    public JumpCircle() {
        addSettings(maxRadius);
    }
    ...
    private void doCircle(MatrixStack stack, final Vector3d pos, double maxRadius, float deltaTime, int index) {
        ...
        float radius = (float) (easeOutCirc(valWave2(1.F - deltaTime)) * (maxRadius * 1.5));
    ...
    private int getColor(int index, float alphaPC) {
        int colorize = 0;
        if (Hud.themeMode.is("Шаблон")) {
            colorize = ColorUtils.getColor(index);
        } else if (Hud.themeMode.is("Кастом")) {
           colorize = Hud.themeColor.getValue();
        }
